...
function trackLabel(track: 'primary' | 'secondary'): string {
  const t = subs.tracks[track]
  if (t.source === 'off') return '關'
  const base = t.source === 'live' ? '即時' : (subs.files.value.find((f) => f.id === t.source)?.name ?? '字幕檔')
  if (t.translateTo !== 'off') {
    const lang = LANGS.find((l) => l.value === t.translateTo)?.label ?? t.translateTo
    return `${base}→${lang}`
  }
  return base
}
  ...
  <div v-if="open" class="sm-pop" @click.self="close">
    <div class="sm-menu" @click.stop>
      <template v-if="screen === 'root'">
        ...
        <div class="item" @click="go('audiosource')"><span class="ic"><PlayerIcon name="mic" :size="18" /></span><span class="nm">外部音源</span><span class="cur">{{ audioSourceLabel() }}</span><span class="gt">›</span></div>
        <div class="sec">字幕</div>
        <div class="item" @click="go('primary')"><span class="ic"><PlayerIcon name="captions-live" :size="18" /></span><span class="nm">主字幕</span><span class="cur" :lang="subs.tracks.primary.translateTo">{{ trackLabel('primary') }}</span><span class="gt">›</span></div>
        <div class="item" :class="{ disabled: secondaryDisabled }" @click="!secondaryDisabled && go('secondary')">
          <span class="ic"><PlayerIcon name="tracks" :size="18" /></span><span class="nm">第二字幕</span><span class="cur" :lang="subs.tracks.secondary.translateTo">{{ secondaryDisabled ? '需先開主字幕' : trackLabel('secondary') }}</span><span class="gt">›</span>
        </div>
      </template>
